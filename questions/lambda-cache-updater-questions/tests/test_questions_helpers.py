...
async def test_get_questions_success(mocker):
    ...
    mocker.patch.dict("os.environ", {"QUESTIONS_API_URL": "http://mockapi.com"})
    ...
    mock_async_client.get.side_effect = [mock_easy_response, mock_hard_response]
...
async def test_get_questions_request_error(mocker):
    ...
    with pytest.raises(WeeklyQuestionsError) as exc:
        await get_questions()
    assert "Failed to contact external API: Connection failed" in str(exc.value)
...
async def test_get_questions_invalid_json(mocker):
    ...
    mock_easy_response = MagicMock(status_code=200)
    mock_easy_response.json.side_effect = ValueError("No JSON object could be decoded")
    ...
    with pytest.raises(WeeklyQuestionsError) as exc:
        await get_questions()
    assert "Invalid JSON response from external API: No JSON object could be decoded" in str(exc.value)
...
def test_format_questions_data():
    ...
    with patch("get_questions.get_day_start") as mock_get_day_start:
        mock_get_day_start.return_value = datetime.datetime(2025, 3, 2, 0, 0, 0)
