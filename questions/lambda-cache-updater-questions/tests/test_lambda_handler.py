...
async def test_async_handler_success(mocker, fake_valkey_client):
    ...
    fake_payload = {"timestamp": "2025-03-02T00:00:00", "questions": fake_questions}

    # Patch the helper functions.
    mocker.patch("lambda_handler.get_questions", new=AsyncMock(return_value=fake_questions))
    mocker.patch("lambda_handler.format_questions_data", return_value=fake_payload)
...
async def test_async_handler_initialization_failure(mocker):
    ...
    mocker.patch("lambda_handler.GlideClient.create", side_effect=Exception("Init failure"))
    result = await async_handler({}, {})
    assert result["statusCode"] == 500
    body = json.loads(result["body"])
