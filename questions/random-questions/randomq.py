...
def generate_random_questions(count=7, difficulty='introductory', source='leetcode'):
    ...
    if len(questions) < count:
        print("DEBUG: Not enough unseen questions; resetting seen status and incrementing id for all rows.")
        ...
        for q in all_questions:
            new_id = q["id"] + 1000
            update_response = supabase.table("questions_test")\
        ...
        if data.get("error"):
            error_msg = data["error"].get("message", "Unknown error")
            raise RuntimeError(f"Error fetching questions after update: {error_msg}")
        questions = data.get("data", [])
        if len(questions) < count:
            raise ValueError("Still not enough questions available even after resetting seen status.")
