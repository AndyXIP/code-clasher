...
def format_leaderboard_entry(entry: dict, rank: int) -> dict:
    ...
    Args:
        entry (dict): A dictionary representing a leaderboard row. Expected keys include 'name' and 'score'.
        rank (int): The rank (position) of this entry.

    Returns:
        dict: A formatted dictionary with added 'rank' and cleaned up 'name' and 'score' values.
    """
...
def format_leaderboard_data(raw_data: dict) -> dict:
    ...
    Args:
        raw_data (dict): The raw leaderboard data expected to have keys "easy" and "hard", each being a list
                         of entries (dictionaries with at least 'name' and 'score').
    ...
    for category in ["easy", "hard"]:
        ...
        # Optionally, sort the entries (if not already sorted). This example sorts descending by score.
